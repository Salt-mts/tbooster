<?php
    ...
    if(isset($_GET['deact'])){
       ...
       if($deactivate){
        Helper::redirect("engagement?brandID=$brandID&upd=Successful");
       }
    }
                ...
                <div class="dashboard__main__content__row">
                   ...
                   <img src="./assets/img/brand/<?= $brand->logo() ?>" alt="<?= $brand->name() ?>" width="40"> <?= $brand->name() ?></h1> 
                    ...
                    <div class="brand-action" style="display: flex; justify-content: end;">
                        <a href="engagement?brandID=<?= $brandID ?>&delBrand=" onclick="return confirm('Sure to delete?')" class="btn btn-muted">Delete this Brand</a>
                    </div>
                </div>
            </div>
        </div>
